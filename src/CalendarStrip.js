...
class CalendarStrip extends Component {
  ...
  constructor(props) {
    ...
    const selectedDate = this.setLocale(dayjs(this.props.selectedDate));
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!this.compareDates(prevProps.selectedDate, this.props.selectedDate)) {
      updateState = true;
      selectedDate = {
        selectedDate: this.setLocale(dayjs(this.props.selectedDate))
      };
      startingDate = {
  ...
  updateWeekData(
    ...
    for (let i = 0; i < this.numDaysInWeek; i++) {
      let date;
      date = me.setLocale(startingDate.clone().add(i, "day"));
